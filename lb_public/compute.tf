...
resource "oci_core_instance" "lb-vm-instance-01-ol7" {
  ...
  subnet_id = "${oci_core_subnet.public-lb-subnet-AD1.id}"
  metadata = {
...
resource "oci_core_instance" "lb-vm-instance-02-ol7" {
  ...
  shape = "${var.instance_shape}"
  subnet_id = "${oci_core_subnet.public-lb-subnet-AD2.id}"
  metadata = {
    ssh_authorized_keys = "${file(var.ssh_public_key_file_ol7)}"
    user_data = "${base64encode(file(var.BootStrapFile_ol7))}"
  }
...
output "Public-IP-of-instance-01" {
  value = ["${oci_core_instance.lb-vm-instance-01-ol7.*.public_ip}"]
}
...
output "Public-IP-of-instance-02" {
  value = ["${oci_core_instance.lb-vm-instance-02-ol7.*.public_ip}"]
}
